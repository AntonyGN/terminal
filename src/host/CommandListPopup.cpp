...
void CommandListPopup::_drawList()
{
...

    {
        ...
        for (til::CoordType i = 0; i < Height(); ++i)
        {
            ...
            _screenInfo.GetTextBuffer().Write(WriteCoord.y, false, _attributes, state);
            WriteCoord.y += 1;
        }
    }
    ...
    for (; i <= _bottomIndex; i++)
    {
        ...
        _screenInfo.GetTextBuffer().Write(WriteCoord.y, false, attr, state);
        _screenInfo.GetTextBuffer().ReplaceAttributes(WriteCoord.y, state.columnEnd, state.columnLimit, attr);
