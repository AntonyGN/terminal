...
namespace
{
    struct BufferAllocator
    {
        BufferAllocator(til::size sz)
        {
            ...
            // We add +1 to the height, because TextBuffer will use the first rowStride to construct _scratchRow.
    ...
    _isActiveBuffer{ isActiveBuffer }
{
    ...
    _scratchRow = { allocator.chars(), allocator.indices(), allocator.width(), _currentAttributes };
    ++allocator;
...
try
{
    ...
    ROW scratchRow{ allocator.chars(), allocator.indices(), allocator.width(), _currentAttributes };
    ++allocator;

    std::vector<ROW> newStorage;
    newStorage.reserve(allocator.height());
    for (til::CoordType i = 0; i < newSize.height; ++i, ++allocator)
    {
        newStorage.emplace_back(allocator.chars(), allocator.indices(), allocator.width(), _currentAttributes);
    }

    // This basically imitates a std::rotate_copy(first, mid, last), but uses CopyRangeFrom().
    {
        ...
        for (const auto& sourceRange : sourceRanges)
        {
            for (const auto& oldRow : sourceRange)
            {
                ...
                dest->TransferAttributes(oldRow.Attributes(), allocator.width());
                ++dest;
            }
        }
    }
    ...
    _scratchRow = std::move(scratchRow);
